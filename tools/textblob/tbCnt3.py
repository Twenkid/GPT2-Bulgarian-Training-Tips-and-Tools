...
paths = [str(x) for x in Path("E:\\Corpus\\").glob("*.txt")] 
texts = {}
blobs = {}
counts = {}
sortedCounts = {}
...
def FreqAll(): #bAddDict=False):
    #File by file
    x = ''
    fo = open("outnltk_all.txt", "wt", encoding='utf-8')
    ...
    curr = ''
    print("Prepare text output...")
    ##for w in sorted(analysis, reverse=True): #alphabet     
    ##  curr+=w + "\t" + str(analysis[w]) + "\n"  
    ##  counts[w] = analysis[w]
    #sortedCounts = dict(sorted(counts, key=lambda item: item[1]))
    #if (bAddDict):
    sortedCounts = dict(sorted(analysis.items(), key=lambda item: item[1], reverse=True)    
